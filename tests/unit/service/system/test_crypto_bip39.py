...
import pytest
from vldmcp.service.system.crypto import CryptoService, ed25519_keypair_from_seed, generate_node_id
...
def test_save_and_load_key(tmp_path):
    ...
    assert crypto_service.load_key(missing_path) is None
...
def test_save_key_invalid_size(tmp_path):
    ...
    with pytest.raises(ValueError, match="Key must be exactly 32 bytes"):
        crypto_service.save_key(b"too short", key_path)

    with pytest.raises(ValueError, match="Key must be exactly 32 bytes"):
        crypto_service.save_key(b"x" * 33, key_path)
...
def test_generate_mnemonic_and_key():
    ...
    assert crypto_service.is_valid_mnemonic(mnemonic)
...
def test_key_from_mnemonic_invalid():
    ...
    with pytest.raises(ValueError, match="Invalid mnemonic phrase"):
        crypto_service.key_from_mnemonic(bad_mnemonic)
...
def test_mnemonic_from_key_invalid_size():
    ...
    with pytest.raises(ValueError, match="Key must be exactly 32 bytes"):
        crypto_service.mnemonic_from_key(b"too short")

    with pytest.raises(ValueError, match="Key must be exactly 32 bytes"):
        crypto_service.mnemonic_from_key(b"x" * 31)


def test_is_valid_mnemonic():
    """Test mnemonic validation."""
    # Generate a valid mnemonic
    mnemonic, _ = crypto_service.generate_mnemonic_and_key()
    assert crypto_service.is_valid_mnemonic(mnemonic) is True

    # Test invalid cases
    assert crypto_service.is_valid_mnemonic("not a valid mnemonic") is False
    assert crypto_service.is_valid_mnemonic("abandon " * 24) is False  # Bad checksum
    assert crypto_service.is_valid_mnemonic("") is False
...
def test_consistent_mnemonic_generation():
    ...
    mnemonic1 = crypto_service.mnemonic_from_key(key)
    mnemonic2 = crypto_service.mnemonic_from_key(key)
...
def test_different_keys_different_mnemonics():
    ...
    mnemonic1 = crypto_service.mnemonic_from_key(key1)
    mnemonic2 = crypto_service.mnemonic_from_key(key2)
...
def test_real_bip39_test_vector():
    ...
    mnemonic = crypto_service.mnemonic_from_key(test_key)
    assert mnemonic.startswith(expected_words_start)

    # Round-trip should work
    recovered = crypto_service.key_from_mnemonic(mnemonic)
    assert recovered == test_key


def test_ed25519_keypair_generation():
    """Test Ed25519 keypair generation."""
    seed = crypto_service.generate_key()
    public_key, private_key = ed25519_keypair_from_seed(seed)
...
def test_ed25519_invalid_seed_size():
    ...
    with pytest.raises(ValueError, match="Seed must be 32 bytes"):
        ed25519_keypair_from_seed(b"too short")
...
def test_integration_full_identity_lifecycle(storage_service):
    ...
    mnemonic1, key1 = crypto_service.generate_mnemonic_and_key()
    user_key_path = storage_service.user_key_path()
    crypto_service.save_key(key1, user_key_path)

    # 2. Export seed phrase
    loaded_key = crypto_service.load_key(user_key_path)
    exported_mnemonic = crypto_service.mnemonic_from_key(loaded_key)
    ...
    recovered_key = crypto_service.key_from_mnemonic(exported_mnemonic)
    crypto_service.save_key(recovered_key, user_key_path)

    # 5. Verify recovery worked
    final_key = crypto_service.load_key(user_key_path)
